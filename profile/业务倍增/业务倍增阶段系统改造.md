...
这里使用 Elasticsearch 来作为搜索引擎，业务侧 insbiz 保存不变，产生数据后入库。新增一个定时任务通过 MySQL 只读实例轮询变更的数据，然后推送到消息队列中，新增一个服务 sync-es 来同步这些数据到 Elasticsearch 中。在运营管理系统 opsmgt-backend 中则直接通过 Elasticsearch 来检索。可以根据业务要求，灵活调整同步的频率，可以做到准实时同步。
